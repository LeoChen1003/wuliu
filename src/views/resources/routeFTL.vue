            ...
            <div style="background:#fff;box-sizing:border-box;padding:20px;">
              ...
              >
                ...
                <el-form-item :label="$t('resources.truckType')">
                  {{ truckObj[thisRow.truck.category] }} -
                  {{ subObj[thisRow.truck.subCategory] }}
                </el-form-item>
              ...
              <el-table border :data="priceList">
                ...
                <el-table-column header-align="center" align="center" :label="$t('resources.distance_KM')">
                  <template slot-scope="scope">
                    <div>
                      {{
                        scope.row.minMiles === scope.row.maxMiles
                          ? scope.row.minMiles
                          : scope.row.minMiles + "~" + scope.row.maxMiles
                      }}
                    </div>
                ...
                <template slot-scope="scope">
                  <div>
                    <el-select
                      ...
                      @visible-change="
                        status => {
                          canChoose(status, scope.$index);
                        }
                      "
                      multiple
                    >
                      <el-option-group
                        v-for="(group, index) in waitTo_cityList"
